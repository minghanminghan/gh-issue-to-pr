...
|------|------|------|
| **0 Setup** | Deterministic | Fetches issue via `gh`, clones/verifies repo, creates `agent/<hash>` branch, writes `ISSUE.md` and `STATE.json` |
...
```
python main.py <issue_url> <repo_url> [options]
```
...
|------|-----------|----------|
| `STATE.json` | All steps | Shared pipeline state — branch name, loop counts, cost, PR URL, etc. |
...
| `tests/test_tools.py` | 22 | All `fs` tool primitives (`read_file`, `write_file`, `create_file`, `append_file`, `list_dir`, `grep`), shell allowlist checks, `execute_cli` safety |
...
│   ├── shell.py             # execute_cli with allowlist enforcement
│   ├── state.py             # STATE.json read/write, init_run
│   ├── setup.py             # Step 0: issue fetch, clone, branch creation
│   ├── report.py            # Step 6: FAILURE.md, TRACE.json, sys.exit
...
│
├── tests/                   # pytest test suite (82 tests, no external deps)
│
├── docs/
...
- **No container isolation** — `execute_cli` is allowlisted but commands run on the host

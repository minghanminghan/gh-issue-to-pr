"""Tests for tool manifests and LiteLLM/OpenAI schema generation."""
...
class TestBuildTools:
    ...
    def test_execute_cli_schema_has_cmd_property(self):
        schemas = build_tools(EXECUTE_MANIFEST)
        execute_cli_schema = next(s for s in schemas if s["function"]["name"] == "execute_cli")
        assert "cmd" in execute_cli_schema["function"]["parameters"]["properties"]
        assert "cmd" in execute_cli_schema["function"]["parameters"]["required"]

...
def test_run_subcommand_invalid_guidelines(tmp_path):
    args = argparse.Namespace(
        ...
        guidelines="invalid_guidelines.md",
        config=None,
        max_steps=None,
        budget=2.0
    )
...
def test_serve_subcommand(mock_run):
    ...
    mock_run.assert_called_once_with("server:app", host="0.0.0.0", port=8080, reload=False)


@patch("main._run_subcommand")
def test_main_run(mock_run_subcommand):
    test_args = ["main.py", "run", "https://github.com/owner/repo/issues/1", "--budget", "5.0"]
